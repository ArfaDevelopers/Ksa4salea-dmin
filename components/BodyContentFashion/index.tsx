...
interface BodyContentFashionItem {
  id: string;
  content: string;
...
const BodyContentFashion: React.FC = () => {
  ...
  const [aboutList, setAboutList] = useState<BodyContentFashionItem[]>([]);
  const [editId, setEditId] = useState<string | null>(null);

  const fetchData = async () => {
    try {
      const querySnapshot = await getDocs(collection(db, "BodyContentFashion"));
      const data = querySnapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      })) as BodyContentFashionItem[];
      setAboutList(data);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editId) {
        ...
        await updateDoc(doc(db, "BodyContentFashion", editId), {
          content: input,
        });
        ...
        await addDoc(collection(db, "BodyContentFashion"), {
          content: input,
  ...
  const handleEdit = (item: BodyContentFashionItem) => {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (result.isConfirmed) {
      await deleteDoc(doc(db, "BodyContentFashion", id));
