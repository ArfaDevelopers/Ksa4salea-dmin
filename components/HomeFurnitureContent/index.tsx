...
interface HomeFurnitureContentItem {
  id: string;
  content: string;
...
const HomeFurnitureContent: React.FC = () => {
  ...
  const [aboutList, setAboutList] = useState<HomeFurnitureContentItem[]>([]);
  const [editId, setEditId] = useState<string | null>(null);

  const fetchData = async () => {
    try {
      const querySnapshot = await getDocs(
        collection(db, "HomeFurnitureContent")
      );
      const data = querySnapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      })) as HomeFurnitureContentItem[];
      setAboutList(data);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editId) {
        ...
        await updateDoc(doc(db, "HomeFurnitureContent", editId), {
          content: input,
        });
        ...
        await addDoc(collection(db, "HomeFurnitureContent"), {
          content: input,
  ...
  const handleEdit = (item: HomeFurnitureContentItem) => {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (result.isConfirmed) {
      await deleteDoc(doc(db, "HomeFurnitureContent", id));
