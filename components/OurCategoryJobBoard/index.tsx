...
const OurCategoryJobBoard = () => {
  ...
  useEffect(() => {
    const fetchAds = async () => {
      try {
        const adsCollection = collection(db, "OurCategoryJobBoardAutomative"); // Get reference to the 'Cars' collection
    ...
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          ...
          await deleteDoc(doc(db, "OurCategoryJobBoardAutomative", ad.id)); // Delete document by id
  ...
  const handleAddCarUpdatedd = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const carDocRef = doc(db, "OurCategoryJobBoardAutomative", _id); // _id is accessible from closure
  ...
  const handleEditClick = async (ad: any) => {
    ...
    try {
      const adDoc = await getDoc(
        doc(db, "OurCategoryJobBoardAutomative", ad.id)
      );
      if (adDoc.exists()) {
  ...
  const handleAddCar = async (e: any) => {
    ...
    try {
      ...
      const carsCollection = collection(db, "OurCategoryJobBoardAutomative");
...
export default OurCategoryJobBoard;
