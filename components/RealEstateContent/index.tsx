...
interface RealEstateContentItem {
  id: string;
  content: string;
  createdAt: Timestamp | Date;
}

const RealEstateContent: React.FC = () => {
  const MySwal = withReactContent(Swal);
  const editor = useRef(null);

  const [input, setInput] = useState<string>("");
  const [aboutList, setAboutList] = useState<RealEstateContentItem[]>([]);
  const [editId, setEditId] = useState<string | null>(null);

  const fetchData = async () => {
    try {
      const querySnapshot = await getDocs(collection(db, "RealEstateContent"));
      const data = querySnapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      })) as RealEstateContentItem[];
      setAboutList(data);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editId) {
        ...
        await updateDoc(doc(db, "RealEstateContent", editId), {
          content: input,
        });
        ...
        await addDoc(collection(db, "RealEstateContent"), {
          content: input,
  ...
  const handleEdit = (item: RealEstateContentItem) => {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (result.isConfirmed) {
      await deleteDoc(doc(db, "RealEstateContent", id));
