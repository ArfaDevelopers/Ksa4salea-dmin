...
const OurCategoryPetAnimals = () => {
  ...
  useEffect(() => {
    const fetchAds = async () => {
      try {
        const adsCollection = collection(db, "OurCategoryPetAnimalsAutomative"); // Get reference to the 'Cars' collection
    ...
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          ...
          await deleteDoc(doc(db, "OurCategoryPetAnimalsAutomative", ad.id)); // Delete document by id
  ...
  const handleAddCarUpdatedd = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const carDocRef = doc(db, "OurCategoryPetAnimalsAutomative", _id); // _id is accessible from closure
  ...
  const handleEditClick = async (ad: any) => {
    ...
    try {
      const adDoc = await getDoc(
        doc(db, "OurCategoryPetAnimalsAutomative", ad.id)
      );
      if (adDoc.exists()) {
    ...
    const fetchCars = async () => {
      try {
        ...
        const carsCollectionRef = collection(
          db,
          "OurCategoryPetAnimalsAutomative"
        );
  ...
  const handleAddCar = async (e: any) => {
    ...
    try {
      ...
      const carsCollection = collection(db, "OurCategoryPetAnimalsAutomative");
...
export default OurCategoryPetAnimals;
