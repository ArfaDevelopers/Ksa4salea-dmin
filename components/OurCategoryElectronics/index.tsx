...
const OurCategoryElectronics = () => {
  ...
  useEffect(() => {
    const fetchAds = async () => {
      try {
        const adsCollection = collection(db, "OurCategoryElectronics"); // Get reference to the 'Cars' collection
    ...
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          ...
          await deleteDoc(doc(db, "OurCategoryElectronics", ad.id)); // Delete document by id
  ...
  const handleAddCarUpdatedd = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const carDocRef = doc(db, "OurCategoryElectronics", _id); // _id is accessible from closure
  ...
  const handleEditClick = async (ad: any) => {
    ...
    try {
      const adDoc = await getDoc(doc(db, "OurCategoryElectronics", ad.id));
      if (adDoc.exists()) {
  ...
  useEffect(() => {
    const fetchCars = async () => {
      try {
        ...
        const carsCollectionRef = collection(db, "OurCategoryElectronics");
  ...
  const handleAddCar = async (e: any) => {
    ...
    try {
      ...
      const carsCollection = collection(db, "OurCategoryElectronics");
...
export default OurCategoryElectronics;
