...
const OurCategoryHealthCare = () => {
  ...
  useEffect(() => {
    const fetchAds = async () => {
      try {
        const adsCollection = collection(db, "OurCategoryHealthCare"); // Get reference to the 'Cars' collection
    ...
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          ...
          await deleteDoc(doc(db, "OurCategoryHealthCare", ad.id)); // Delete document by id
  ...
  const handleAddCarUpdatedd = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const carDocRef = doc(db, "OurCategoryHealthCare", _id); // _id is accessible from closure
  ...
  const handleEditClick = async (ad: any) => {
    ...
    try {
      const adDoc = await getDoc(doc(db, "OurCategoryHealthCare", ad.id));
      if (adDoc.exists()) {
  ...
  useEffect(() => {
    const fetchCars = async () => {
      try {
        ...
        const carsCollectionRef = collection(db, "OurCategoryHealthCare");
  ...
  const handleAddCar = async (e: any) => {
    ...
    try {
      ...
      const carsCollection = collection(db, "OurCategoryHealthCare");
...
export default OurCategoryHealthCare;
