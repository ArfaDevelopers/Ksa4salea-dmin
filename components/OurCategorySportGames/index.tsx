...
const OurCategorySportGames = () => {
  ...
  useEffect(() => {
    const fetchAds = async () => {
      try {
        const adsCollection = collection(db, "OurCategorySportGamesAutomative"); // Get reference to the 'Cars' collection
    ...
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          ...
          await deleteDoc(doc(db, "OurCategorySportGamesAutomative", ad.id)); // Delete document by id
  ...
  const handleAddCarUpdatedd = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const carDocRef = doc(db, "OurCategorySportGamesAutomative", _id); // _id is accessible from closure
  ...
  const handleEditClick = async (ad: any) => {
    ...
    try {
      const adDoc = await getDoc(
        doc(db, "OurCategorySportGamesAutomative", ad.id)
      );
      if (adDoc.exists()) {
    ...
    const fetchCars = async () => {
      try {
        ...
        const carsCollectionRef = collection(
          db,
          "OurCategorySportGamesAutomative"
        );
  ...
  const handleAddCar = async (e: any) => {
    ...
    try {
      ...
      const carsCollection = collection(db, "OurCategorySportGamesAutomative");
...
export default OurCategorySportGames;
