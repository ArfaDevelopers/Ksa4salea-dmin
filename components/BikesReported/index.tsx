...
const BikesReported = () => {
  ...
  const handleRevertClick = async (id: string) => {
    try {
      const adsCollection = collection(db, "Bikes");
  ...
  useEffect(() => {
    const fetchAds = async () => {
      try {
        const adsCollection = collection(db, "Bikes"); // Get reference to the 'Cars' collection
  ...
  const handleEditClick = async (id: any) => {
    ...
    try {
      const adDoc = await getDoc(doc(db, "Bikes", id));
      if (adDoc.exists()) {
    ...
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          ...
          await deleteDoc(doc(db, "Bikes", ad.id)); // Delete document by id
  ...
  const handleAddCar = async (e: any) => {
    ...
    try {
      ...
      const carsCollection = collection(db, "Bikes");
          ...
          <div className="flex items-center justify-center min-h-screen">
            <div className="bg-white rounded-lg shadow-lg w-full max-w-md mx-4 p-6 relative">
              ...
              <h3 className="text-3xl font-semibold text-center text-gray-800 mb-6">
                {selectedAd ? "Update a Electronic" : "Add a Electronic"}
              </h3>
