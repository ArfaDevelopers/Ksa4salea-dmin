...
const OurCategoryTravel = () => {
  ...
  useEffect(() => {
    const fetchAds = async () => {
      try {
        const adsCollection = collection(db, "OurCategoryTravelAutomative"); // Get reference to the 'Cars' collection
    ...
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          ...
          await deleteDoc(doc(db, "OurCategoryTravelAutomative", ad.id)); // Delete document by id
  ...
  const handleAddCarUpdatedd = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const carDocRef = doc(db, "OurCategoryTravelAutomative", _id); // _id is accessible from closure
  ...
  const handleEditClick = async (ad: any) => {
    ...
    try {
      const adDoc = await getDoc(doc(db, "OurCategoryTravelAutomative", ad.id));
      if (adDoc.exists()) {
  ...
  useEffect(() => {
    const fetchCars = async () => {
      try {
        ...
        const carsCollectionRef = collection(db, "OurCategoryTravelAutomative");
  ...
  const handleAddCar = async (e: any) => {
    ...
    try {
      ...
      const carsCollection = collection(db, "OurCategoryTravelAutomative");
...
export default OurCategoryTravel;
