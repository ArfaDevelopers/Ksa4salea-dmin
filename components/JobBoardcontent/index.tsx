...
interface JobBoardcontentItem {
  id: string;
  content: string;
  createdAt: Timestamp | Date;
}

const JobBoardcontent: React.FC = () => {
  const MySwal = withReactContent(Swal);
  const editor = useRef(null);

  const [input, setInput] = useState<string>("");
  const [aboutList, setAboutList] = useState<JobBoardcontentItem[]>([]);
  const [editId, setEditId] = useState<string | null>(null);

  const fetchData = async () => {
    try {
      const querySnapshot = await getDocs(collection(db, "JobBoardcontent"));
      const data = querySnapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      })) as JobBoardcontentItem[];
      setAboutList(data);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editId) {
        ...
        await updateDoc(doc(db, "JobBoardcontent", editId), {
          content: input,
        });
        ...
        await addDoc(collection(db, "JobBoardcontent"), {
          content: input,
  ...
  const handleEdit = (item: JobBoardcontentItem) => {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (result.isConfirmed) {
      await deleteDoc(doc(db, "JobBoardcontent", id));
