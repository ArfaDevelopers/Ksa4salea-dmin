...
const REALESTATECOMP = () => {
  ...
  const [ads, setAds] = useState<Ad[]>([]); // D
  ...
  useEffect(() => {
    const fetchAds = async () => {
      try {
        ...
        // Fetch views
        const viewsDocRef = doc(db, "views", "reakestate");
  ...
  //   const fetchAds = async () => {
  //     try {
  //       const adsCollection = collection(db, "REALESTATECOMP"); // Get reference to the 'Cars' collection
  ...
  const handleEditClick = async (id: any) => {
    ...
    try {
      const adDoc = await getDoc(doc(db, "REALESTATECOMP", id));
      if (adDoc.exists()) {
    ...
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          ...
          await deleteDoc(doc(db, "REALESTATECOMP", ad.id)); // Delete document by id
  ...
  const handleAddCar = async (e: any) => {
    ...
    try {
      ...
      const carsCollection = collection(db, "REALESTATECOMP");
            ...
            {filteredAds.map((ad) => (
              ...
              >
                ...
                <td className="px-6 py-4">
                  ...
                  <a
                    href={`http://168.231.80.24:3002/#/Dynamic_Route?id=${ad.id}&callingFrom=RealEstateComp`}
                    target="_blank"
