...
interface SportGamesContentItem {
  id: string;
  content: string;
  createdAt: Timestamp | Date;
}

const SportGamesContent: React.FC = () => {
  const MySwal = withReactContent(Swal);
  const editor = useRef(null);

  const [input, setInput] = useState<string>("");
  const [aboutList, setAboutList] = useState<SportGamesContentItem[]>([]);
  const [editId, setEditId] = useState<string | null>(null);

  const fetchData = async () => {
    try {
      const querySnapshot = await getDocs(collection(db, "SportGamesContent"));
      const data = querySnapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      })) as SportGamesContentItem[];
      setAboutList(data);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editId) {
        ...
        await updateDoc(doc(db, "SportGamesContent", editId), {
          content: input,
        });
        ...
        await addDoc(collection(db, "SportGamesContent"), {
          content: input,
  ...
  const handleEdit = (item: SportGamesContentItem) => {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (result.isConfirmed) {
      await deleteDoc(doc(db, "SportGamesContent", id));
