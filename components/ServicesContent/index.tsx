...
const ServicesContent: React.FC = () => {
  ...
  const [aboutList, setAboutList] = useState<ServicesContentItem[]>([]);
  const [editId, setEditId] = useState<string | null>(null);

  const fetchData = async () => {
    try {
      const querySnapshot = await getDocs(collection(db, "ServicesContent"));
      const data = querySnapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      })) as ServicesContentItem[];
      setAboutList(data);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editId) {
        ...
        await updateDoc(doc(db, "ServicesContent", editId), {
          content: input,
        });
        ...
        await addDoc(collection(db, "ServicesContent"), {
          content: input,
  ...
  const handleEdit = (item: ServicesContentItem) => {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (result.isConfirmed) {
      await deleteDoc(doc(db, "ServicesContent", id));
